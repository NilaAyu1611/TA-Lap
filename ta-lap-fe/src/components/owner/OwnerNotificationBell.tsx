...
export default function OwnerNotificationBell() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      /* ignore when offline / unauthorized */
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        className="relative flex h-11 w-11 items-center justify-center rounded-2xl border border-gray-200 bg-white transition hover:border-violet-500/40 dark:border-white/10 dark:bg-white/5"
        ...
        <div className="absolute right-0 top-full z-50 mt-2 w-[min(100vw-2rem,380px)] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-white/10 dark:bg-[#0f172a]">
              ...
              items.map((item) => (
                <button
                  ...
                  className={`w-full border-b border-gray-50 px-4 py-3 text-left transition hover:bg-gray-50 dark:border-white/5 dark:hover:bg-white/5 ${
                    !item.is_read ? "bg-violet-50/50 dark:bg-violet-500/5" : ""
                  }`}
                >
          ...
          <div className="border-t border-gray-100 px-4 py-2 dark:border-white/10">
            ...
            <Link
              ...
              className="block py-2 text-center text-xs font-medium text-violet-600 hover:text-violet-500 dark:text-violet-400"
            >
              Lihat semua pesanan
            </Link>
          </div>
