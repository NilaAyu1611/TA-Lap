...
import { LaporanBulanan } from "@/types/laporan";
...
export default function LaporanBulananTable({ data }: Props) {
  if (data.length === 0) {
    return (
  ...
  const maxPemasukan = Math.max(...data.map((d) => d.pemasukan), 1);
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
      <div className="border-b border-gray-100 px-4 py-3 dark:border-white/5">
        <h3 className="text-sm font-semibold">Trend Pemasukan Bulanan</h3>
        <p className="text-xs text-gray-500">Komisi platform per bulan</p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/80 text-left text-xs uppercase text-gray-500 dark:border-white/5 dark:bg-white/[0.04]">
              ...
              <th className="px-4 py-3">Pemasukan Admin</th>
              <th className="px-4 py-3 w-40">Grafik</th>
            </tr>
          </thead>
                ...
                <td className="px-4 py-3">
                  <div className="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-white/10">
                    <div
                      ...
                      style={{
                        width: `${Math.round((row.pemasukan / maxPemasukan) * 100)}%`,
                      }}
                    />
                  </div>
                </td>
